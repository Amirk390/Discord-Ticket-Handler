...
class Overlay:
    def __init__(self, root):
        ...
        self.title_frame = tk.Frame(self.container, bg='#641d77')
        self.title_frame.pack(pady=(10, 0), fill=tk.X)

        self.manual_button = tk.Button(self.title_frame, text="?", command=self.toggle_manual, bg='#A98BC4', fg='white', font=("Helvetica", 10, "bold"), width=2)
        self.manual_button.pack(side=tk.LEFT, padx=(10, 0))

        self.title_label = tk.Label(self.title_frame, text="Purple RP Ticket Assistant", bg='#641d77', fg='white', font=("Helvetica", 16, "bold"))
        self.title_label.pack(pady=0, expand=True)
        ...
        self.create_extra_button("Provided video has no context")
        ...
        # Initialize manual window variable
    ...
    def create_extra_button(self, text):
        button = tk.Button(self.extra_buttons_frame, text=text.capitalize(), command=lambda t=text: self.update_big_box(t.capitalize() + '.'), bg='#A98BC4', fg='white')
        button.pack(side=tk.LEFT, padx=5)

    def toggle_manual(self):
        if self.manual_window is None:
            self.manual_window = tk.Frame(self.container, bg='black')
            self.manual_window.place(relx=0.5, rely=0.5, anchor=tk.CENTER, relwidth=0.9, relheight=0.8)

            self.manual_label = tk.Label(self.manual_window, text="Purple RP Ticket Assistant - User Manual", bg='black', fg='white', font=("Helvetica", 12, "bold"))
            self.manual_label.pack(pady=(10, 10))
